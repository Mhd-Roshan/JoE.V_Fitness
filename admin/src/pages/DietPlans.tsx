...
export default function DietPlans() {
    ...
    useEffect(() => {
        ...
        async function load() {
            try {
                // 1. Fetch Templates, Assignments, Users, and Subscriptions concurrently
                // We limit assignments to 20 temporarily so we have enough valid ones after filtering deleted users
                const [snap, assignSnap, usersSnap, subsSnap] = await Promise.all([
                    getDocs(collection(db, "dietPlanTemplates")),
                    getDocs(query(collection(db, "clientDietPlans"), orderBy("assignedAt", "desc"), limit(20))),
                ...
                // 2. Build User Map for exact names, avatars, and user-level durations
                const userMap: Record<string, UserData> = {};
                usersSnap.docs.forEach((doc) => {
                    userMap[doc.id] = doc.data() as UserData;
                });

                // 3. Build Subscription Map to get actual Package Durations
                const subMap: Record<string, string> = {};
                subsSnap.docs.forEach((doc) => {
                    const data = doc.data();
                    if (data.clientId && (data.status === "active" || data.status === "Active")) {
                        subMap[data.clientId] = data.duration || data.packageDuration || (data.durationWeeks ? `${data.durationWeeks} Weeks` : "");
                    }
                });

                const monthStart = startOfMonth();

                // Build a quick count of active assignments per templateId in-memory
                const templateAssignCountMap: Record<string, number> = {};
                assignSnap.docs.forEach((d) => {
                    const data = d.data();
                    if (data.templateId && (data.status === "active" || data.status === "Active")) {
                        templateAssignCountMap[data.templateId] = (templateAssignCountMap[data.templateId] || 0) + 1;
                    }
                });

                // 4. Process Templates instantly
                const rows = snap.docs.map((d) => {
                    ...
                    return {
                        ...
                        createdAt,
                        assignedCount: templateAssignCountMap[d.id] || 0,
                    };
                });
                ...
                setTemplates(rows);
                setTotalAssignedCount(assignSnap.docs.filter(d => d.data().status === "active").length);
                setNewThisMonth(rows.filter((r) => r.createdAt && r.createdAt >= monthStart).length);

                // 5. Process Assignments for Table (Filtering out deleted users & getting actual durations)
                const finalAssignments: AssignmentRow[] = [];

                for (const d of assignSnap.docs) {
                    const data = d.data();
                    const clientId = data.clientId;

                    // STRICT CHECK: If client doesn't exist in users map (Deleted by Admin), SKIP entirely.
                    if (!clientId || !userMap[clientId]) continue;

                    const userMatch = userMap[clientId];
                    const cName = userMatch.fullName || userMatch.name;

                    // Safety check: must have a real name
                    if (!cName) continue;

                    // Determine accurate package duration (Sub collection > User profile > Fallback)
                    const durationStr = subMap[clientId]
                        || userMatch.packageDuration
                        || (data.durationWeeks ? `${data.durationWeeks} Weeks` : "—");
                    ...
                    finalAssignments.push({
                        id: d.id,
                        clientName: cName, // Exact live name from profile
                        clientPhoto: cPhoto,
                        ...
                        assignedDate: assignedAt
                            ...
                            : "—",
                        duration: durationStr, // Actual package duration
                        status: data.status ?? "active",
                    });

                    // Stop once we have 5 valid, non-deleted assignments for the UI table
                    if (finalAssignments.length >= 5) break;
                }
                            ...
                            {assignments.length === 0 ? (
                                <tr>
                                    <td colSpan={5} className="empty-state" style={{ padding: "32px" }}>
                                        No active plans found for existing clients.
                                    </td>
                                </tr>
                            ) : (
                                assignments.map((a) => (
